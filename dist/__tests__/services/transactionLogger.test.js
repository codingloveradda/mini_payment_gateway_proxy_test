...
describe('Transaction Logger Service', () => {
    beforeEach(() => {
        ...
        const allTransactions = transactionLogger_1.transactionLogger.getAllTransactions();
    ...
    describe('logTransaction', () => {
        it('should log successful transaction', () => {
            ...
            transactionLogger_1.transactionLogger.logTransaction(1000, 'USD', 'user@example.com', 'tok_live_123', 0.3, 'success', 'stripe', 'stripe_123456', 'Transaction processed successfully', metadata);
            const transactions = transactionLogger_1.transactionLogger.getAllTransactions();
        ...
        it('should log blocked transaction', () => {
            ...
            transactionLogger_1.transactionLogger.logTransaction(15000, 'USD', 'user@test.ru', 'tok_test_123', 0.8, 'blocked', null, null, 'Transaction blocked due to high risk', metadata);
            const transactions = transactionLogger_1.transactionLogger.getAllTransactions();
            expect(transactions.length).toBeGreaterThan(0);
            const lastTransaction = transactions[0];
    ...
    describe('getAllTransactions', () => {
        it('should return transactions in reverse chronological order', () => {
            ...
            for (let i = 1; i <= 3; i++) {
                transactionLogger_1.transactionLogger.logTransaction(100 * i, 'USD', `user${i}@example.com`, 'tok_live_123', 0.3, 'success', 'stripe', `stripe_${i}`, 'Transaction processed', { processingTime: 100 });
            }
            const transactions = transactionLogger_1.transactionLogger.getAllTransactions();
    ...
    describe('getTransactionsByStatus', () => {
        it('should filter transactions by status', () => {
            ...
            transactionLogger_1.transactionLogger.logTransaction(1000, 'USD', 'user@example.com', 'tok_live_123', 0.3, 'success', 'stripe', 'stripe_123', 'Success', { processingTime: 100 });
            transactionLogger_1.transactionLogger.logTransaction(15000, 'USD', 'user@test.ru', 'tok_test_123', 0.8, 'blocked', null, null, 'Blocked', { processingTime: 200 });
            const successfulTransactions = transactionLogger_1.transactionLogger.getTransactionsByStatus('success');
            const blockedTransactions = transactionLogger_1.transactionLogger.getTransactionsByStatus('blocked');
